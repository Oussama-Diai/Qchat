...
void FenClient::on_boutonEnvoyer_clicked()
{
    ...
    message->setFocus(); // Et on remet le curseur ? l'intérieur
}

// Appuyer sur la touche Entrée a le m?me effet que cliquer sur le bouton "Envoyer"
...
{
    /* M?me principe que lorsque le serveur reçoit un paquet :
...
void FenClient::erreurSocket(QAbstractSocket::SocketError erreur)
{
    switch(erreur) // On affiche un message différent selon l'erreur qu'on nous indique
    {
        case QAbstractSocket::HostNotFoundError:
            listeMessages->append(tr("<em>ERREUR : le serveur n'a pas pu ?tre trouve. Verifiez l'IP et le port.</em>"));
            break;
        case QAbstractSocket::ConnectionRefusedError:
            listeMessages->append(tr("<em>ERREUR : le serveur a refuse la connexion. Verifiez si le programme \"serveur\" a bien ete lance. Verifiez aussi l'IP et le port.</em>"));
            break;
        case QAbstractSocket::RemoteHostClosedError:
            listeMessages->append(tr("<em>ERREUR : le serveur a coupe la connexion.</em>"));
            break;
        default:
